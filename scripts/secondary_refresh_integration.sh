...
#   - a secondary pointed at the primary via Secondary.Zones + Primary.Ip/Port
#
# Asserts the secondary converges on the zone:
#   A) at STARTUP, without any NOTIFY (startup sweep -> AXFR)
...
start_primary() {
    ...
    api "$PRIMARY_SOCKET" PATCH /api/config \
        '{"mode":"primary","allow_axfr":true,"dnssec_enabled":false,"enforce_tsig":false}' >/dev/null \
...
# poll_secondary_serial <expected_serial> <deadline_seconds>
poll_secondary_serial() {
    ...
    local got
    while (( SECONDS < deadline )); do
        got="$(soa_serial "$SECONDARY_DNS_PORT")"
...
main() {
    ...
    echo "== Assertion A: startup refresh without NOTIFY =="
    ...
    echo "[A] ok: secondary serves $ZONE at startup (serial=$PRIMARY_SERIAL, www=$SEC_WWW), no NOTIFY"
